...
export const SkuggleAIBuddy: React.FC<SkuggleAIBuddyProps> = ({
  ...
  return (
    <>
      {/* Collapsed floating robot — expands into chat on click */}
      <div className="fixed bottom-20 sm:bottom-6 right-5 z-40">
        <motion.button
          whileHover={{ scale: 1.08 }}
          whileTap={{ scale: 0.94 }}
          onClick={() => setIsOpen(true)}
          className="relative flex h-14 w-14 items-center justify-center rounded-full bg-indigo-900 text-white shadow-xl border border-indigo-700 hover:shadow-indigo-900/40 transition-shadow"
          aria-label="Open Ask Skuggle chat"
          title="Ask Skuggle"
        >
          <span className="absolute inset-0 rounded-full bg-amber-400/30 animate-ping" aria-hidden="true" />
          <span className="absolute inset-1 rounded-full bg-violet-500/20 animate-pulse" aria-hidden="true" />
          <span className="relative z-10 [&_.animate-pulse]:animate-[pulse_1.1s_ease-in-out_infinite]">
            <RobotGraphic size="sm" />
          </span>
          <span className="sr-only">Ask Skuggle — AI Teaching & Learning</span>
        </motion.button>
      </div>
