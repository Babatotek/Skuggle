...
Route::prefix('v1')->group(function (): void {
    ...
    Route::middleware(['auth:sanctum', 'tenant', 'throttle:api'])->group(function (): void {
        ...
        Route::middleware(['verified', 'mfa'])->group(function (): void {
            ...
            Route::get('/assessments/templates', [AssessmentController::class, 'templates']);
            ...
            Route::match(['put', 'patch'], '/assessments/{assessment}', [AssessmentController::class, 'update'])->middleware(['permission:assessment.create', 'idempotency:required']);
            ...
            Route::post('/assessments/{assessment}/duplicate', [AssessmentController::class, 'duplicate'])->middleware(['permission:assessment.create', 'idempotency:required']);
            Route::post('/assessments/{assessment}/template', [AssessmentController::class, 'saveTemplate'])->middleware(['permission:assessment.create', 'idempotency:required']);
            Route::post('/assessments/{assessment}/apply-template', [AssessmentController::class, 'applyTemplate'])->middleware(['permission:assessment.create', 'idempotency:required']);
            Route::match(['get', 'put'], '/assessments/{assessment}/sections', [AssessmentController::class, 'sections']);
            Route::post('/assessment-questions/{question}/media', [AssessmentQuestionController::class, 'media'])->middleware(['permission:assessment.create', 'throttle:uploads', 'idempotency:required']);
            Route::get('/assessment-questions/{question}/media', [AssessmentQuestionController::class, 'showMedia']);
            Route::get('/smartmark/batches/{batch}/export', [SmartmarkController::class, 'export'])->middleware('permission:assessments.view');
            ...
            Route::get('/employees/lookups', [EmployeeController::class, 'lookups'])->middleware('permission:users.manage');
            Route::get('/employees/imports/template', [EmployeeImportController::class, 'template'])->middleware('permission:users.manage');
            Route::post('/employees/imports/validate', [EmployeeImportController::class, 'validateUpload'])->middleware(['permission:users.manage', 'throttle:uploads']);
            Route::post('/employees/imports/confirm', [EmployeeImportController::class, 'confirm'])->middleware(['permission:users.manage', 'idempotency:required']);
            Route::post('/workforce/positions', [EmployeeController::class, 'storePosition'])->middleware(['permission:users.manage', 'idempotency:required']);
            Route::get('/employees/{employee}', [EmployeeController::class, 'show'])->middleware('permission:users.manage');
            Route::get('/employees/{employee}/documents', [EmployeeDocumentController::class, 'index'])->middleware('permission:users.manage');
            Route::post('/employees/{employee}/documents', [EmployeeDocumentController::class, 'store'])->middleware(['permission:users.manage', 'throttle:uploads', 'idempotency:required']);
            Route::get('/employees/{employee}/documents/{document}/download', [EmployeeDocumentController::class, 'download'])->middleware('permission:users.manage');
            Route::post('/employees/{employee}/access', [TenantMembershipController::class, 'employeeAccess'])->middleware(['permission:roles.manage', 'idempotency:required']);
            Route::get('/school/access-catalog', [TenantMembershipController::class, 'catalog'])->middleware('permission:roles.manage');
            Route::post('/school/access-roles', [TenantMembershipController::class, 'storeAccessRole'])->middleware(['permission:roles.manage', 'idempotency:required']);
            Route::patch('/school/access-roles/{accessRole}', [TenantMembershipController::class, 'updateAccessRole'])->middleware(['permission:roles.manage', 'idempotency:required']);
            Route::delete('/school/access-roles/{accessRole}', [TenantMembershipController::class, 'destroyAccessRole'])->middleware(['permission:roles.manage', 'idempotency:required']);
