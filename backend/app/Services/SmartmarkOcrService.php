...
final class SmartmarkOcrService
{
    ...
    {
        if (config('skuggle.ocr.provider') !== 'gemini') {
            throw new ApiException('OCR_NOT_CONFIGURED', 'Configure OCR_PROVIDER=gemini and GEMINI_API_KEY before processing scans.', 503);
        }
        ...
        return collect($decoded['sheets'])->map(function (array $sheet) use ($answerKey, $maxScore) {
            $answers = array_values((array) ($sheet['answers'] ?? []));
            $correct = 0;
            foreach ($answerKey as $i => $answer) {
                if (strtoupper((string) ($answers[$i] ?? '?')) === strtoupper((string) $answer)) {
                    $correct++;
                }
            }$score = count($answerKey) ? round($correct / count($answerKey) * $maxScore, 2) : 0;

            return ['admission_number' => $sheet['admissionNumber'] ?? null, 'student_name' => $sheet['studentName'] ?? null, 'answers' => $answers, 'detected_score' => $score, 'confidence' => max(0, min(100, (float) ($sheet['confidence'] ?? 0))), 'human_review_required' => (bool) ($sheet['humanReviewRequired'] ?? true) || ((float) ($sheet['confidence'] ?? 0) < (float) config('skuggle.ocr.review_threshold', 92)), 'flag_reason' => $sheet['flagReason'] ?? null];
        })->all();
    }
}
