...
final class CbtController extends Controller
{
    ...
    {
        $privileged = (bool) app(\App\Domain\Tenancy\TenantContext::class)->membership()->role->privileged;
    ...
    public function submit(Request $request,string $quiz): JsonResponse
    {
        ...
        $data=$request->validate(['answers'=>['required','array']]); $questions=$q->questions; $correct=0;
        foreach($questions as $question) if(($data['answers'][$question['id']]??null)===($question['correctOptionId']??null)) $correct++;
